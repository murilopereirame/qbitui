...
export class QBitAPI {
  ...
  async getTransferInfo(): Promise<TransferInfo> {
    ...
    return res.json() as Promise<TransferInfo>;
  }
  ...
  async getCategories(): Promise<Record<string, { name: string; savePath: string }>> {
    ...
    if (!res.ok) return {};
    return res.json() as Promise<Record<string, { name: string; savePath: string }>>;
  }
  ...
  /**
   * Add a torrent file using a React Native file object (URI, name, type).
   * FormData on React Native accepts { uri, name, type } objects directly.
   */
  async addTorrentFileFromUri(
    uri: string,
    fileName: string,
    ...
    form.append("torrents", { uri, name: fileName, type: "application/x-bittorrent" } as unknown as Blob);
    this.applyOptions(form, options);
    const res = await fetch(this.url("/api/v2/torrents/add"), {
      method: "POST",
      headers: this.headers,
